#!/usr/bin/env python3
"""
train_deepsdf_arch_reconstruction_v2.py
...
6.  LR schedule: MultiStepLR with milestones [3500, 9500] and gamma 0.5,
    giving 1e-4 → 5e-5 → 2.5e-5. Milestones mirror the PIINN curriculum
    stage boundaries so both methods receive equivalent optimization
    conditions. Without this, the baseline failed to converge within the
    13500-step budget (best loss reached only at step 13493/13500).

7.  All other infrastructure (Volume, coordinate transforms, export,
...
formulation. A multi-step learning rate schedule was applied with decay
milestones at steps 3500 and 9500 (gamma=0.5), mirroring the stage
boundaries of the proposed PIINN curriculum to ensure equivalent
optimization conditions across all compared methods."

Changelog vs v1
---------------
- [FIX] Added MultiStepLR schedule (milestones=[3500, 9500], gamma=0.5)
  to match PIINN optimization conditions and ensure convergence within
  the shared 13500-step budget.
- [FIX] CSV now logs current LR per step (lr_current column).
- [FIX] qc_metrics.json now records lr_schedule parameters.
- [FIX] Logging prints current LR at each reporting step.
...
TRAIN_CONFIG = {
    ...
    "steps": 13500,
    "lr_initial": 1e-4,
    # LR schedule — milestones mirror PIINN stage boundaries for fair comparison
    # 1e-4 → 5e-5 (at step 3500) → 2.5e-5 (at step 9500)
    "lr_milestones": [3500, 9500],
    "lr_gamma": 0.5,
    "band_mm": 1.0,
    "n_total": 45000,
...
# ============================================================
# Sampling utilities  (identical to PIINN and Vanilla INR)
# ============================================================
...
def one_train_step(
    ...
    # DeepSDF uses raw normalized coordinates — NO positional encoding
...
def train_case(
    ...
    # --- Optimizer ---
    opt = torch.optim.Adam(model.parameters(), lr=float(TRAIN_CONFIG["lr_initial"]))

    # --- LR schedule ---
    # MultiStepLR with milestones matching PIINN stage boundaries:
    #   step 3500: 1e-4 → 5e-5  (end of PIINN stage 1)
    #   step 9500: 5e-5 → 2.5e-5 (end of PIINN stage 2)
    # This gives the DeepSDF baseline equivalent optimization pressure
    # across the 13500-step shared budget.
    scheduler = torch.optim.lr_scheduler.MultiStepLR(
        opt,
    ...
    logging.info(
        f"LR schedule: MultiStepLR milestones={TRAIN_CONFIG['lr_milestones']} "
        f"gamma={TRAIN_CONFIG['lr_gamma']} "
        f"(1e-4 → 5e-5 at step 3500 → 2.5e-5 at step 9500)"
    )
    ...
    with open(metrics_path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow([
            "global_step", "loss_total", "loss_sdf",
            "loss_sdf_inner", "loss_sdf_outer",
            "lr_current",          # [NEW] actual LR at this step
            "n_total", "band_mm", "clamp_delta_mm", "w_inner", "w_outer",
        ])
    ...
    logging.info(
        f"Training DeepSDF-Arch v2 case: {case_dir.name} on {device} | "
        f"steps={steps} lr_init={TRAIN_CONFIG['lr_initial']} "
        f"n_total={n_total} band={band_mm}mm clamp={clamp_delta_mm}mm "
        f"w_inner={w_inner} w_outer={w_outer}"
    )
    ...
    for step in range(1, steps + 1):
        ...
        # Step the LR scheduler AFTER the optimizer step
        scheduler.step()
        ...
        with open(metrics_path, "a", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow([
                global_step, loss_total, stats["sdf"],
        ...
        if loss_total < best_loss:
            ...
            save_ckpt(ckpt_dir / "best.pt", model, opt, scheduler,
                      global_step, best_loss, TRAIN_CONFIG)

        if save_every > 0 and (step % save_every == 0):
            save_ckpt(ckpt_dir / f"iter_{global_step:06d}.pt", model, opt, scheduler,
                      global_step, best_loss, TRAIN_CONFIG)

    save_ckpt(ckpt_dir / "last.pt", model, opt, scheduler,
              global_step, best_loss, TRAIN_CONFIG)
    ...
    qc: Dict[str, object] = {
        ...
        "method":                "DeepSDF-Architecture Baseline v2",
        ...
        "lr_schedule": {
            ...
            "gamma":      float(TRAIN_CONFIG["lr_gamma"]),
            "lr_values":  [
                float(TRAIN_CONFIG["lr_initial"]),
                float(TRAIN_CONFIG["lr_initial"]) * float(TRAIN_CONFIG["lr_gamma"]),
                float(TRAIN_CONFIG["lr_initial"]) * float(TRAIN_CONFIG["lr_gamma"]) ** 2,
            ],
            "rationale": (
                "Milestones mirror PIINN curriculum stage boundaries (3500, 9500) "
                "to ensure equivalent optimization conditions for fair comparison."
            ),
        },
    ...
    if export_final:
        ...
        logging.info(f"{case_dir.name} | Exported FINAL DeepSDF-Arch v2 outputs to: {case_out}")
    else:
        logging.info(f"{case_dir.name} | Training complete. Final export skipped.")

    with open(case_out / "qc_metrics.json", "w", encoding="utf-8") as f:
        json.dump(qc, f, indent=2)

    logging.info(
        f"{case_dir.name} | DONE | best_loss={best_loss:.6f} "
        f"train_time={train_time_sec/60:.1f}min"
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "DeepSDF-architecture baseline v2 for dual-SDF coronary reconstruction. "
            "Adds MultiStepLR schedule mirroring PIINN stage boundaries."
        )
    )
    ap.add_argument("--case-dir",     required=True, type=str,
                    help="Path to a single case folder.")
    ...
    ap.add_argument("--export-final", action="store_true",
                    help="Export final SDF grids, meshes, and QC at the end.")
    ap.add_argument("--save-every",   type=int, default=0,
                    help="Save iter_XXXXXX.pt every N steps. 0 disables.")
