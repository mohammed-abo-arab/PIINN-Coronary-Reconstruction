...
TRAIN_CONFIG = {
    ...
    "band_mm": 2.0,
    "n_total": 45000,
    # Pure SDF sampling: inner band, outer band, background.
    # Sum must be <= 1.0
    "r_inner": 0.35,
    "r_outer": 0.35,
    "r_bg": 0.30,
...
def train_case(
    ...
    with open(metrics_path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["global_step", "loss_total", "loss_sdf", "lr", "n_total", "band_mm"])
    ...
    logging.info(
        ...
        f"steps={steps} lr={lr} n_total={n_total} band={band_mm}mm | loss=SDF only"
    )
    ...
    for step in range(1, steps + 1):
        ...
        with open(metrics_path, "a", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow([global_step, loss_total, stats["sdf"], lr, n_total, band_mm])
    ...
    qc: Dict[str, object] = {
        ...
        "time_per_iter_ms": (1000.0 * train_time_sec / max(global_step, 1)),
        ...
        "frozen_parameters": int(n_nontrainable),
        "model_size_mb": float(model_size_mb),
        "training_config": TRAIN_CONFIG,
        "training_objective": "L1 SDF regression only",
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Train corrected Vanilla INR for dual-SDF coronary reconstruction (single-stage, SDF loss only)."
    )
