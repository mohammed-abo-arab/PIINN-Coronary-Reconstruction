...
# No-curriculum ablation: one fixed operating point for the full 13,500 steps.
# 46,481 * 13,500 = 627,493,500 samples, essentially identical to the
# full curriculum budget of 627,500,000 samples.
NO_CURRICULUM = [
    {
        "name": "single_stage_no_curriculum",
        "steps": 13500,
        "lr": 5e-5,
        "band_mm": 1.5,
        "n_total": 46481,
...
def one_train_step(
    ...
    if uniform_sampling:
        # Ablation: remove region-targeted sampling while preserving n_total.
        x_mm = sample_uniform_physical(center, half, device, n_total)
    else:
    ...
    # Compute Eikonal regularization only when it is active.
    # In the no_eikonal ablation it is completely excluded from both
    # computation and the optimization objective.
    if weights["eik"] > 0.0:
    ...
    # Anatomy-specific losses are evaluated only when active. In the
    # no_anatomical ablation they are completely excluded from computation
    # and therefore cannot contribute to the optimization graph.
    if any(weights[k] > 0.0 for k in ("enc", "order", "minT", "smoothT", "lumen_in")):
        ...
        loss_smoothT = smooth_thickness_loss_masked(
            phi_in, phi_out, x_norm, vol_in_sdf, wall_mask
        )
        loss_lumen_in = masked_mean(F.relu(phi_in), lumen_mask)
    else:
        loss_enc = torch.tensor(0.0, device=device)
        loss_order = torch.tensor(0.0, device=device)
        loss_minT = torch.tensor(0.0, device=device)
        loss_smoothT = torch.tensor(0.0, device=device)
        loss_lumen_in = torch.tensor(0.0, device=device)
...
def train_case(
    ...
    active_curriculum = NO_CURRICULUM if ablation == "no_curriculum" else CURRICULUM

    model = MLP2SDF(hidden=256, layers=6, pe_freqs=10).to(device)
    opt = torch.optim.Adam(model.parameters(), lr=active_curriculum[0]["lr"])
    ...
    logging.info(f"Training case: {case_dir.name} on {device} | ablation={ablation} | stages={len(active_curriculum)}")
    ...
    for cfg in active_curriculum:
        ...
        # Component-wise ablation: all non-ablated components remain unchanged.
        uniform_sampling = (ablation == "no_region_sampling")

        if ablation == "no_eikonal":
            weights["eik"] = 0.0

        elif ablation == "no_anatomical":
            # Anatomy-specific constraints removed; SDF, Eikonal and BC retained.
            weights["enc"] = 0.0
            weights["order"] = 0.0
            weights["minT"] = 0.0
            weights["smoothT"] = 0.0
            weights["lumen_in"] = 0.0
...
def main() -> None:
    ...
    ap.add_argument(
        "--ablation",
        choices=["full", "no_eikonal", "no_anatomical", "no_region_sampling", "no_curriculum"],
        default="full",
        help="Component-wise ablation; 'full' reproduces the original PIINN."
    )
    args = ap.parse_args()
