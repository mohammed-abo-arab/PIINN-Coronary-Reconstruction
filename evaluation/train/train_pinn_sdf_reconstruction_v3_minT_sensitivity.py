...
def build_curriculum(minT_scale: float) -> List[dict]:
    """
    Return an experiment-specific copy of CURRICULUM in which only w_minT
    is scaled. All other architecture, sampling, optimization, and loss
    parameters remain identical to the submitted PIINN configuration.

    minT_scale = 0.0  -> w_minT = [0.00, 0.00, 0.00]
    minT_scale = 0.5  -> w_minT = [0.05, 0.14, 0.11]
    minT_scale = 1.0  -> w_minT = [0.10, 0.28, 0.22] (original PIINN)
    minT_scale = 2.0  -> w_minT = [0.20, 0.56, 0.44]
    """
    if minT_scale < 0:
        raise ValueError("minT_scale must be non-negative.")
    curriculum = copy.deepcopy(CURRICULUM)
    for cfg in curriculum:
        cfg["w_minT_original"] = float(cfg["w_minT"])
        cfg["w_minT"] = float(cfg["w_minT"]) * float(minT_scale)
        cfg["minT_scale"] = float(minT_scale)
    return curriculum
...
def train_case(
    ...
    logging.info(f"Training case: {case_dir.name} on {device} | stages={len(curriculum)} | minT_scale={minT_scale:g}x")
    ...
    qc = {
        ...
        "minT_scale": float(minT_scale),
        "effective_w_minT_by_stage": {cfg["name"]: float(cfg["w_minT"]) for cfg in curriculum},
        "original_w_minT_by_stage": {cfg["name"]: float(cfg.get("w_minT_original", cfg["w_minT"])) for cfg in curriculum},
        "total_training_steps": int(sum(int(cfg["steps"]) for cfg in curriculum)),
...
# ============================================================
def main() -> None:
    ap = argparse.ArgumentParser(description="Dual-SDF PIINN reconstruction with controlled w_minT sensitivity analysis.")
    ...
    ap.add_argument(
        ...
        help=(
            "Sensitivity multiplier applied ONLY to the stage-wise minimum-separation loss weight w_minT. "
            "0=disabled, 0.5=half, 1=original PIINN, 2=double. All other parameters remain unchanged."
        ),
    )
    ...
    if requested_device.type == "cuda" and not torch.cuda.is_available():
        ...
        if device.type == "cuda":
            # Make CUDA accounting and allocations refer to the explicitly requested GPU.
            torch.cuda.set_device(device)

    curriculum = build_curriculum(args.minT_scale)
    logging.info(
        "Minimum-separation sensitivity: scale=%gx | effective w_minT=%s",
        args.minT_scale,
        [float(cfg["w_minT"]) for cfg in curriculum],
    )
